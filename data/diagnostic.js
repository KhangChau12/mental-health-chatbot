...
export function getDisorderInfo(disorder) {
  return diagnosticCriteria[disorder] || null;
}

export function formatDisorderInfo(disorder) {
  const info = getDisorderInfo(disorder);
  if (!info) return "";
  ...
  if (info.duration) {
    message += `### Thời gian: ${info.duration}\n\n`;
  }
  
  if (info.impairment) {
    message += `### Ảnh hưởng: ${info.impairment}\n\n`;
  }
  
  message += "### Mức độ nghiêm trọng\n";
  Object.entries(info.severity).forEach(([level, description]) => {
    message += `- **${level}**: ${description}\n`;
  });
  message += "\n";
  
  message += "### Các phương pháp điều trị có thể\n";
  info.treatmentOptions.forEach(option => {
    message += `- ${option}\n`;
  });
