...
export function getResourcesForSeverity(category, severity) {
  let result = [];
  
  // Luôn thêm tài nguyên chung
  result = result.concat(resources.general);
  
  // Thêm tài nguyên theo danh mục và mức độ
  if (resources[category] && resources[category][severity]) {
    result = result.concat(resources[category][severity]);
  }
  
  // Nếu mức độ nghiêm trọng, thêm thông tin khẩn cấp
  if (severity === 'severe' || severity === 'moderatelySevere' || severity === 'extremelySevere') {
    result = result.concat(resources.emergency);
  }
  
  return result;
}

export function formatResourcesMessage(resourceList) {
  let message = "Đây là một số tài nguyên có thể hữu ích cho bạn:\n\n";
  
  resourceList.forEach((resource, index) => {
    message += `${index + 1}. **${resource.title}**\n`;
    if (resource.description) message += `   ${resource.description}\n`;
    if (resource.examples) message += `   Ví dụ: ${resource.examples}\n`;
    if (resource.phone) message += `   Liên hệ: ${resource.phone}\n`;
    if (resource.link) message += `   Tham khảo: ${resource.link}\n`;
    message += "\n";
  });
